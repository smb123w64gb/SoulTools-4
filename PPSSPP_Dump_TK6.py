...
BREAKPOINT_ADDR = 0x08a9dcb0   # Example: a common default load address for user memory
MEMORY_ADDR = 0x09B0E3C0       # Memory address to read
READ_LENGTH = 0x20000               # Number of bytes to read

fileout = open('data.bin','wb')
    ...
    async with websockets.connect(WS_URL) as websocket:
        ...
        async for message in websocket:
            ...
            if data.get("event") == "cpu.stepping" and not startRead:
                ...
                if(data.get("uintValue") == 0x20000 or data.get("uintValue") == 0x2800):
                    print(data)
                    read_memory_message = {
                        "event": "memory.read",
                        "address": MEMORY_ADDR,
