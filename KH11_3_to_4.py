...
entrySize = entryCount*0x48
attackCount = int((grabDMG-attackList)/0x58)
attackSize = attackCount*0x60
...
for x in range(attackCount):#I guess the reversed them????
    ...
    for y in range(34):
        s16(inf,ouf)
    ret = inf.tell()
    ouf.write(b'\x00\x00\x00\x00\x00\x00')
    #3 (6 bytes) more for IV
    ouf.write(b'\x00\x00\x00\x00')
    #inf.seek(4,1)
    s16(inf,ouf)
    s16(inf,ouf)
    ouf.write(inf.read(4))
    s16(inf,ouf)
    inf.seek(2,1)#SC3 padding?
inf.seek(grabDMG)
